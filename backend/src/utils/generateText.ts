...
export const generateTextDirect = async (prompt: string) => {
  console.log(genAI);
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          try {
            ...
            if (
              ...
              reject(
                new Error(
                  "Gemini response did not contain expected text content."
                )
              );
            }
          } catch (e) {
            reject(
              new Error(
                `Failed to parse JSON response from Gemini: ${e.message}`
              )
            );
          }
        } else {
          reject(
            new Error(
              `Gemini API Error: Status ${res.statusCode}, Message: ${data}`
            )
          );
        }
      });
