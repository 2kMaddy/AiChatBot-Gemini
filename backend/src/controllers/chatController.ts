...
import {
  ...
  updateChat,
} from "../utils/createAndUpdateChat.js";
...
export const getResponsebyPrompt = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const deleteAllChatByUserId = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const openChatById = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const chat = chatsList.find(
      (session) => session.sessionId === chatSessionId
    );
    if (chat) {
